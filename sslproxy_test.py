...
class TestClient(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.ca = self._temp_file + 'testCA.pem'
    self.cert = self._temp_file + 'testCA-cert.cer'
    self.wrong_ca = self._temp_file + 'wrong.pem'
    self.wrong_cert = self._temp_file + 'wrong-cert.cer'
  ...
  def verify_cb(self, conn, cert, errnum, depth, ok):
    ...
    self.assertTrue(time.strftime('%Y%m%d%H%M%SZ',
                    time.gmtime()) > cert.get_notBefore())
    return ok
